...
class User extends BaseUser
{
     ...
     *      inverseJoinColumns={@ORM\JoinColumn(name="vote_user_id", referencedColumnName="id", onDelete="CASCADE")}
     * )
     */
    ...
    public function getSalt() {
    	...
    	if($this->salt == null && $this->id != null) {
    		...
    		\App::getInstance()->getTable("AppBundle:User")->updateSalt($this->id, $this->salt);
    	}
    ...
    {
    	return $this->getNumConnections() > 0;
    }

    public function getFriends()
    {
    	return \App::getTable("AppBundle:Network")->getConnections($this->id);
    }

    public function getNumConnections()
    {
    	return \App::getTable("AppBundle:Network")->getNumConnections($this->id);
    }

    public function hasMedia()
    {
    	return $this->getNumMedia() > 0;
    }
 
    public function getNumMedia()
    {
    	return \App::getTable("AppBundle:User")->getNumMedia($this->id);
    }

    public function getPhotosAndVideos() {
    	return \App::getTable("AppBundle:User")->getPhotosAndVideos($this->id);
    }

    public function getExperiences() {
    	return \App::getTable("AppBundle:Experience")->getList($this->id);
    }

    public function isVotedBy($user_id) {
    	return \App::getTable("AppBundle:User")->isVotedByUser($this->id, $user_id);
    }

    public function getNumVotes() {
    	return \App::getTable("AppBundle:User")->numVotes($this->id);
    }

    public function getVoteList() {
    	return \App::getTable("AppBundle:User")->listVoted($this->id);
    }

    public function getRoles() {
    	$user_roles = parent::getRoles();
    ...
    private $mutualFriends = null;

    public function getHasMutualFriends() {
     	return count($this->getMutualFriends()) > 0;
    }

    public function getMutualFriends() {
    	if($this->mutualFriends == null) {
    		$this->mutualFriends = \App::getTable("AppBundle:Network")->listMutualFriends(\App::getInstance()->getUserId(), $this->id);
    	}
    	return $this->mutualFriends;
    }

    public function getTrade_posts() {
    	return \App::getTable("AppBundle:Tradeland")->getLatestPosts($this->id);
    }
}
