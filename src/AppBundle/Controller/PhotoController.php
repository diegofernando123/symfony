...
class PhotoController extends Controller
{
     ...
     * Lists all Photo entities.
     *
     * @Route("/cover-image", name="cover_image")
     * @Method("POST")
     * @param Request $request
     * @return \Symfony\Component\HttpFoundation\Response
     */
    public function coverImageAction(Request $request)
    {
        ...
        foreach ($request->files as $uploadedFile) {
        	$fileName = $this->get('app.image_uploader')->uploadAndResize($uploadedFile, $this->container->getParameter('cover_directory'), 760, 340);
        	if(strlen($this->getUser()->getCoverImage()) > 0) { 
        		unlink($this->container->getParameter('cover_directory') . "/" . $this->getUser()->getCoverImage());
        	}
          	$em->getRepository('AppBundle:User')->updateCoverImage($this->getUser()->getId(), $fileName);
        }

        return $this->redirectToRoute('user_show', ['user' => $this->getUser()->getId()]);
    }
